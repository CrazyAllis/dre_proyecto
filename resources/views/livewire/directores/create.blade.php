<div>
    <form wire:submit='store' class="space-y-4 max-w-2xl p-4 bg-surface-alt dark:bg-surface-dark-alt rounded-lg shadow-md">
        <x-form.input wire:model='nombres' label="Nombres" name="nombres" placeholder="Ingrese nombres"/>
        <x-form.input wire:model='apellidos' label="Apellidos" name="apellidos" placeholder="Ingrese apellidos"/>
        <x-form.input wire:model='dni' label="DNI" name="dni" type="number" placeholder="Ingrese DNI" />
        <x-form.input wire:model='cargo' label="Cargo" name="cargo" placeholder="Ingrese cargo" />
        <x-form.input wire:model='telefono' label="Teléfono" name="telefono" placeholder="999999999" />
        ...
        <a href="{{ route('directores.index') }}" wire:navigate class="w-fit whitespace-nowrap rounded-radius bg-primary border border-primary px-4 py-2 text-center text-sm font-medium tracking-wide text-on-primary transition hover:opacity-75 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary active:opacity-100 active:outline-offset-0 disabled:cursor-not-allowed disabled:opacity-75 dark:border-primary-dark dark:bg-primary-dark dark:text-on-primary-dark dark:focus-visible:outline-primary-dark" role="button">
        Cancelar
        </a>
    </form>
</div>